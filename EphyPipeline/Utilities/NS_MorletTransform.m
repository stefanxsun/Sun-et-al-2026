function cp2MorletSTR = NS_MorletTransform(db1Trace, inSampleRate, varargin)
%CP2MORLETSTR = NS_MorletTransform(DB1TRACE, INSAMPLERATE, [DB1FREQ, CHWAV])
...
if nargin < 3, db1Freq = 2:2:120; else, db1Freq = varargin{1}; end 
if nargin < 4, chWav = 'cmor1-2'; else, chWav = varargin{2}; end 
...
if inNChunk > 1
    ...
    for iChunk = 1:inNChunk
        cp2ChunkSTR = cwt(db1Trace2(in1PadChunkBeg(iChunk):in1PadChunkEnd(iChunk)), scales, chWav); %Spectro Temporal Representation
        cp2MorletSTR(:,in1ChunkBeg(iChunk):in1ChunkEnd(iChunk)) = cp2ChunkSTR(:, inOLSize + 1: end - inOLSize);
    end
else
    %Case where there is only one chunk
    cp2MorletSTR = cwt(db1Trace,scales, chWav); %Spectro Temporal Representation
end
