...
ellipse_t.rotel = rotated_ellipse;
ellipse_t.new_ver_line = new_ver_line;
ellipse_t.new_horz_line = new_horz_line;

%%%%%%%%%%%% OPTIONAL CODE TO DO THINGS INSTEAD OF THE 10 point distance crit to the light

%     [srtDist,srtIndx] = sort(maxN(:));
%     zz = zeros(1,length(maxN));
%     for k = round(length(srtDist)*3/4):length(srtDist) % the 3/4 ensures we look at the biggest portion of the image
%       try % because we might not have 30, in that case no outlier detection
%          xxx  = 1:30; % look at the last 30 pixels
%          idx = find(~isnan(srtDist));
%          idx = idx(find(idx<k));
%          idx = idx(end-29:end);
%          yyy  = srtDist(idx)';
%       catch
...
%       yyy  = yyy(~isnan(yyy));
%
%       % fit a linear regression and see whether the current point is an outlier
%       phat = polyfit(xxx(:),yyy(:),1);
%       yhat = polyval(phat,1:31);
%
%       % compute the error
%       ee = yhat(:)-[srtDist(k-30:k)];
%       ee(isnan(ee)) = 0;
%
%       % compute the z-score of the errors and take away any >SD*c point
%       zsc = zscore(ee);
%       zz(k) = zsc(end);
%
%       % remove the SD point
...
%      rEdges(zz<(-2)) = [];
%      cEdges(zz<(-2)) = [];
%      for zz = 1:length(rEdges)
%        edges(rEdges(zz),cEdges(zz)) = 2;
%      end
%      edges = double(edges==2);
%      edgeIndx = find(edges);
%      [rEdges,cEdges] = ind2sub(size(im),edgeIndx);