function [varargout] = VEHA_L3_MatchPupilWithCameraTriggers(sCFG, sREC)
...
chSourcePath_2 = fullfile(sCFG.sPARAM.chBaseDirectory, 'VEHA_L2_RejectPupilErrors');
chRPEFile = 'VEHA_L2_RejectPupilErrors.mat';
if ~exist(fullfile(chSourcePath_2, chSessionName, chRPEFile), 'file')
    error('%s does not exist for session %s in %s\r', chRPEFile, chSessionName, chSourcePath_2)
end

%Sets the destination folder
chDestPath = sCFG.sPARAM.chBaseDirectory;
chDestMetaFolder = 'VEHA_L3_MatchPupilWithCameraTriggers';
...
chDestFile = 'VEHA_L3_MatchPupilWithCameraTriggers.mat';
...
sINPUT_2 = load(fullfile(chSourcePath_2, chSessionName, chRPEFile), '-mat');
...
sMOVIE_MT = sINPUT_2.sCFG.sL2RPE.sMOVIE_RE;
inNMovie = length(sMOVIE_MT);
in1MovieLen = zeros(inNMovie, 1);
for ii = 1:inNMovie;
    in1MovieLen(ii) = size(sMOVIE_MT(ii).db2CROutput, 1);
end
...
%     rmdir(fullfile(chDestPath, chDestMetaFolder, chSessionName));
    ...
    [ignore, in1WhichTrigSet] = min(abs(in2DTrig), [], 2); %min distance along the trigger dimension for each movie
...
for ii = 1:inNMovie;
    ...
    if inDropFrame < 0 % if more frames than triggers ...  
        ...
        elseif all(isnan(sMOVIE_MT(ii).db2CROutput(end + inDropFrame + 1:end, 4))); % ... or the pupil matrix has been precomputed too long. In this case the time vector is NaN at the end.
            db1MovieTrigTStamps = cat(2, db1MovieTrigTStamps, nan(1, abs(inDropFrame)));
            chMatchTrigFlag = 'pupil_mat_too_long'; 
    ...
    sMOVIE_MT(ii).inDropFrame = inDropFrame;    
    sMOVIE_MT(ii).db1MovieTrigTStamps = db1MovieTrigTStamps;
    sMOVIE_MT(ii).chMatchTrigFlag = chMatchTrigFlag;
...
sCFG.sINPUT.sL2RPE.chScriptName = sINPUT_2.sCFG.sL2RPE.chScriptName;
sCFG.sINPUT.sL2RPE.chTimeComputed = sINPUT_2.sCFG.sL2RPE.chTimeComputed;

%Writes the output in CSG
sCFG.sL3MPCT.dbITI = dbITI;
sCFG.sL3MPCT.sMOVIE_RE = sMOVIE_MT;
sCFG.sL3MPCT.chScriptName = mfilename('fullpath');
sCFG.sL3MPCT.chTimeComputed = datestr(now);
